...
const Displaypage = (props) => {
  ...
  const [subpage, setSubpage] = React.useState(false);
...
const handlesubpage = (subpageid) => {
  // alert(subpageid)
  localStorage.setItem("subpageid", subpageid)
  // localStorage.setItem("pageid", subpageid)
  props.history.replace("/displaysubpage")
  // setSubpage(true)
}
  React.useEffect(() => {
    ...
    //   console.log("check token: ", csrf)
      ...
      //     }
      // }
    axios.get(`http://localhost:3000/api/v1/subpage/show?pageid=${pageid}`, {headers})
    .then(res => {
        // if(res.data.loggedIn){
        //     setLogged(true)
        // }
    ...
    axios.post("http://localhost:3000/api/v1/getpage", reqData, {headers})
    .then(res => {
        // if(res.data.loggedIn){
        //     setLogged(true)
        // }
    ...
    axios.get(`http://localhost:3000/api/v1/listsubpage?pageid=${id}`, {headers})
    .then(res => {
        // if(res.data.loggedIn){
        //     setLogged(true)
        // }
        console.log("subpagelist: ", res)
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        {!editable && <div id="notEditable" >
            <div>
                ...
                <Button style={{float: "right", marginLeft: "10px"}} onClick={() => {setEditable(true)}} variant="contained">Edit</Button>
                ...
                <p>{pagedata.description}</p><br/>
                    <img style={{width: "100%", height: "100%"}} src={pagedata.image}/><br/><br/><br/><br/>
                    <button style={{float: "right"}}>Create Subpage</button>
                    <h3>Subpages</h3>
                    <ul>
                      {
                        pagelist.map(data => 
                          <li onClick={()=> {handlesubpage(data.id)}}><span style={{cursor: "pointer", color: "blue"}}>{data.title}</span></li>
                          )
                      }
